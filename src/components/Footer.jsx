        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-10 mb-12">
          ...
          <div className="text-center space-y-4">
            <div className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl">
              <i className={`${footerData.brand.icon} text-blue-400 text-2xl`}></i>
          ...
          <div className="text-center space-y-4">
            <h4 className="flex items-center justify-center gap-2 text-lg font-bold text-white mb-4">
              <i className={`${footerData.navigationIcon} text-blue-400`}></i>
              Navigation
            </h4>
            <ul className="flex flex-col items-center gap-3">
              {footerData.navigation.map((item, index) => (
                <li key={index}>
                  <a
                    href={item.href}
                    className="text-gray-300 hover:text-blue-400 transition-all duration-300 text-sm font-medium hover:translate-x-1 inline-block"
                  >
          ...
          <div className="text-center space-y-4">
            <h4 className="flex items-center justify-center gap-2 text-lg font-bold text-white mb-4">
              <i className={`${footerData.socialsIcon} text-blue-400`}></i>
              Find Me Online
            </h4>
              ...
              {footerData.socials.map((social, index) => (
                <li key={index}>
                  <Tippy content={social.label} placement="top">
                    ...
                    >
                      <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 text-white shadow-lg hover:scale-110 hover:bg-gradient-to-br hover:from-blue-500/20 hover:to-purple-500/20 hover:border-blue-400/50 transition-all duration-300">
          ...
          <div className="text-center space-y-4">
            <h4 className="flex items-center justify-center gap-2 text-lg font-bold text-white mb-4">
              <i className="bx bx-mail-send text-blue-400" />
            ...
            <form className="flex flex-col gap-3">
              <input
                ...
                className="px-4 py-3 rounded-xl border-2 border-white/20 bg-white/10 backdrop-blur-sm text-white placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
              />
              <Tippy content="Subscribe to my newsletter!">
                <AnimatedButton
                  ...
                  variant="glow"
                  icon="bx bx-send"
                  className="w-full px-4 py-3 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-400 hover:to-purple-500"
                >
          ...
          <div className="flex gap-6 flex-wrap justify-center">
            {footerData.legalLinks.map((link, index) => (
              <a
                key={index}
                href={link.href}
                className="text-sm text-gray-400 hover:text-blue-400 transition-all duration-300 hover:underline"
              >
                {link.label}
              </a>
            ))}
